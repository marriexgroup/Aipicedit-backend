...
async function generateImageOverlay02(options) {
  const {
    base64Data,
    content,
    pageName,
  ...
  try {
    ...
    // Convert canvas to buffer directly (more efficient than base64)
    ...
    // Convert back to base64 if needed
...
async function drawOverlay(canvas, ctx, image, content, pageName, colors, noTextOverlay, includePageProfileImage, pageProfileImageUrl) {
  ...
  ctx.fillStyle = '#dead5c';
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // Draw image with border offset
  const imageX = borderWidth;
  const imageY = borderWidth;
  const imageWidth = canvas.width - (borderWidth * 2);
  const imageHeight = canvas.height - (borderWidth * 2);
  ...
  const logoWidth = 300;
  const logoHeight = 150; // Changed from square to rectangle aspect ratio
  const logoX = Math.random() < 0.5 ? 40 : 370;
  const logoY = Math.floor(Math.random() * (450 - 150 + 1)) + 40;
  const borderWidth1 = 4;
  const shadowBlur = 10;
  const cornerRadius = 10; // Optional: for rounded corners
  ...
  // Draw the border (white rectangle that will show as border)
  ctx.beginPath();
  // For simple rectangle:
  ctx.rect(logoX, logoY, logoWidth, logoHeight);
  // Or for rounded rectangle:
  // roundedRect(ctx, logoX, logoY, logoWidth, logoHeight, cornerRadius);
  ...
  // Or for rounded rectangle:
  // roundedRect(ctx, logoX + borderWidth1, logoY + borderWidth1, 
  //             logoWidth - (borderWidth1 * 2), logoHeight - (borderWidth1 * 2), 
  //             cornerRadius);
  ctx.closePath();
  ctx.clip();
  ...
  if (!noTextOverlay) {
    ...
    // ctx.fillText('DID YOU KNOW', canvas.width / 2, didYouKnowBoxY + 80);
    ...
    // Draw page name (centered, gold, 10px above border)
    ctx.font = `bold ${pageNameFontSize}px OpenSans-Bold`;
    ctx.fillStyle = pageNameColor;
    ctx.fillText(pageNameText, pageNameX, pageNameY);
  }
}

module.exports = { generateImageOverlay02 };