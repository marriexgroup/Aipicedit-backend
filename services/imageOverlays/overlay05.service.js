...
async function generateImageOverlay05(options) {
  const {
    base64Data,
    content,
    pageName,
  ...
  try {
    // Load image from tempimage.png in the same directory
    const image = await loadImage(path.join(__dirname, 'tempimage.png'));
    const imagelogo = await loadImage(path.join(__dirname, 'logo.png'));
...
module.exports = { generateImageOverlay05 };