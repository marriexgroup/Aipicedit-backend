...
async function generateImageOverlay24(options) {
  const {
    base64Data,
    content,
    pageName,
...
async function drawOverlay(canvas, ctx, image, content, pageName, colors, noTextOverlay, pageProfileImageUrl, includePageProfileImage) {
  ...
  // Apply grayscale filter to the circular logo
  const logoImageData = ctx.getImageData(
    logoX + borderWidth1, 
    logoY + borderWidth1, 
    logoWidth - (borderWidth1 * 2), 
    logoHeight - (borderWidth1 * 2)
  ...
  for (let i = 0; i < logoData.length; i += 4) {
    ...
    const gray = Math.round(0.299 * r + 0.587 * g + 0.114 * b);
    logoData[i] = gray;     // Red
    logoData[i + 1] = gray; // Green
    logoData[i + 2] = gray; // Blue
    // Alpha channel (logoData[i + 3]) remains unchanged
  }
  
  // Put the modified logo image data back
  ctx.putImageData(logoImageData, logoX + borderWidth1, logoY + borderWidth1);
...
module.exports = { generateImageOverlay24 };