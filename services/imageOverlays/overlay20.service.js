...
const { drawSquareImageElement } = require('./comon/squreImageElement');

async function generateImageOverlay20(options) {
  const {
    base64Data,
    content,
    pageName,
...
async function drawOverlay(canvas, ctx, image, content, pageName, colors, noTextOverlay, includePageProfileImage, pageProfileImageUrl) {
  ...
  const wordColors = ['#0af6e8', '#ea1058', '#BC13FE', '#16a0ed', '#FF5E00', '#39FF14', '#FFFF33'];
  const themeColor = wordColors[2];
  ...
  // Draw square image element
    const logoX = Math.random() < 0.5 ? 80 : 900;
    const logoY = Math.floor(Math.random() * (450 - 100 + 1)) + 40;
    drawSquareImageElement(ctx, image,  500, 250, logoX, logoY);

  // Draw logo at top left
  if (includePageProfileImage) {
    const logoImage = await loadImage(pageProfileImageUrl);
    const logoWidth = 120*2;
    ...
    ctx.fillStyle = '#fff'; // Border color (white in this case)
    ctx.fill();
...
module.exports = { generateImageOverlay20 };